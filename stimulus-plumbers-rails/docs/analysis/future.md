...
<div data-controller="accordion">
  <div>
    <button aria-expanded="false" aria-controls="acc-panel-1"
            data-accordion-target="trigger" data-action="click->accordion#toggle keydown->accordion#onKey">
      Reports
    </button>
...
## 2. Combobox Popover Redesign

Replaces `Combobox::Variant` (an immutable struct encoding haspopup/panel_class/popup_id_suffix per hardcoded type) with a builder DSL: `sp_combobox(value:, label:) { |c| c.dropdown/typeahead/date/time/custom }`.

`Combobox::Builder` is yielded to the block. Panel type methods return `nil` (ERB-safe) and store a `Panel::*` instance. Each panel class owns `haspopup`, `popup_id_for(id)`, `trigger_icon`, `readonly_trigger?`, `trigger_opts`, and `render(template, panel_attrs:, popup_id:, label:)`. ARIA on the trigger is derived from the panel instance — no Variant struct involved.

Existing `sp_combobox_*` helpers become thin wrappers delegating to `sp_combobox { |c| c.type(...) }` for backward compatibility.

**Full spec:** [`combobox-popover-redesign.md`](combobox-popover-redesign.md)
